...
/**
 * Fetch a remote image URL and resize it to a JPEG data URL.
 * Falls back to a clear error if the host blocks CORS.
 * @param {string} url
 * @returns {Promise<{ dataUrl: string, width: number, height: number }>}
 */
export async function fetchImageAsDataUrl(url) {
  ...
  try {
    ...
    throw new Error(
      'Could not fetch that image. The host may block cross-origin downloads — try saving the file and uploading it instead.',
    )
  }
